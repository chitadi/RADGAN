...
#         noisy_np = noisy.squeeze(1).detach().cpu().numpy()
#         enh_np = [enhance_low_harmonics_spectral(x, 8000)[0] for x in noisy_np]
#         enh = torch.from_numpy(np.stack(enh_np).astype(np.float32)).to(noisy.device).unsqueeze(1)
#         peak = enh.abs().amax(dim=-1, keepdim=True).clamp_min(1e-8)
#         self._last_peak = peak.detach()
#         enhanced = self.backbone(enh / peak)
#         if enhanced.dim() == 3:
...
#         peak = self._last_peak                  # shape (B,1,1) or (B,1)
#         if peak.dim() == 3:
#             peak = peak.squeeze(1)              # (B,1)
#         enhanced_dn = enhanced * peak           # undo peak norm
#         enhanced_for_metrics = enhanced_dn / self._sf_scale
...
#             self.val_clean_scales.append(peak.detach().cpu().numpy())
#             # scale_clean = batch["scale_clean"]
#             # if isinstance(scale_clean, torch.Tensor):
